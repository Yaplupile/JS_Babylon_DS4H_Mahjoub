...
function createHeroineDude(scene) {
    // load the Dude 3D animated model
     // name, folder, skeleton name 
     BABYLON.SceneLoader.ImportMesh("her", "models/Duded/", "model.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
         let heroineDude = newMeshes[0];
         heroineDude.position = new BABYLON.Vector3(0, 0, 5);  // The original dude
         // make it smaller 
         //heroDude.speed = 0.1;
 
         // give it a name so that we can query the scene to get it by name
         heroineDude.name = "heroineDude";
         ...
         // params = id, speed, scaling, scene
         let heroine = new Dude(heroDude, -1, 0.1, 0.2, scene);
 
         // make clones
         scene.dudeds = [];
         for(let i = 0; i < 10; i++) {
             scene.dudeds[i] = doClone(heroineDude, skeletons, i);
             scene.beginAnimation(scene.dudeds[i].skeleton, 0, 120, true, 1);
...
function moveHeroineDudes() {
    if(scene.dudes) {
        for(var i = 0 ; i < scene.dudes.length ; i++) {
            scene.dudeds[i].Dude.move(scene);
        }
    }    
}
